...
class EditPostActivity extends StatefulWidget {
  ...
  const EditPostActivity({Key key, this.id, this.images, this.content, this.displayName, this.avatar}) : super(key: key);
  @override
  _EditPostActivityState createState() => _EditPostActivityState(id: this.id,images: this.images,content: this.content, displayName: this.displayName, avatar: this.avatar);
}
...
class _EditPostActivityState extends State<EditPostActivity> {
  ...
  _EditPostActivityState({this.id,this.images,this.content,this.displayName,this.avatar});
    ...
    return ListTile(
      ...
      title: GestureDetector(
        child: displayName != null
            ? Text(displayName, style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))
            ...
            IconButton(
                onPressed: () async{
                  if (files != null) {
                    ...
                    await APIService.editPost(EditPostRequestModel(content: contentController.text,addImages: objectName,deleteImages: deleteImages, name: "string" ), this.id);
                    setState(() {
        ...
        children: [
          ...
          GestureDetector(
            child: Row(
              ...
              children: [
                IconButton(onPressed: (){}, icon: Icon(Icons.add_to_photos), color: appPrimaryColor,iconSize: 32,),
                Text("Add more", style: TextStyle(fontSize: 16, color: appPrimaryColor, fontWeight: FontWeight.bold),)
              ],
            ),
            onTap: () async {
  ...
  Widget _imageNetwork (Images image, double height){
    return Container(
