...
class _CreateFoodActivityState extends State<CreateFoodActivity> {
  ...
  List<Step> steps = [Step(stepController: TextEditingController(),index: 0,)];
              ...
              onPressed: () async {
                if (files.length > 0) {
                  ...
                  for (var i in ingredients){
                    if(i.quantityController.text != "" && i.nameController != "" && i.unitController != ""){
                      model.ingredients.add(Ingredients(name: i.nameController.text,unit: i.unitController.text,quantity: num.parse(i.quantityController.text) ));
                    }
                  }
                  for (var i in steps){
                ...
                child: Row(
                  ...
                  children: [
                    IconButton(
                      onPressed: () {
                        setState(() {
                          steps.add(Step(stepController: TextEditingController(), index: steps.length,));
                        });
                      },
                      icon: Icon(Icons.add_circle),
                      color: Colors.black,
        ...
        Flexible(
          ...
          child: TextField(
            onTap: () async {
              ...
              setState(() {
                widget.unitController.text = result;
              });
            },
          ...
          child: TextField(
            onTap: () async {
              final result = await Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => IngredientsName(),
                  ));
              setState(() {
    ...
    return Scaffold(
      ...
      body: circular ? Center(child: CircularProgressIndicator())
       : ListView.builder(
         ...
         return ListTile(title: Text(names[index]), onTap: (){
    ...
    return Scaffold(
      ...
      body: circular ? Center(child: CircularProgressIndicator())
          ...
          itemBuilder: (context, index){
            ...
            return ListTile(title: Text(units[index]), onTap: (){
