import { ForbiddenException, Inject, NotFoundException, Response } from "@nestjs/common";
  ...
  implements IQueryHandler<GetMessagesQuery>
{
  ...
  async execute(query: GetMessagesQuery): Promise<GetMessagesResponse> {
    ...
    const inConversation = await this._conversationRepo.isMember(conversationId, user.id)

    if (!inConversation) {
      throw new ForbiddenException(ResponseDTO.fail("Not in conversation"))
    }

    const [msgs, total] = await this._messageRepo.getMessages(conversationId, queryOptions)

    for (let msg of msgs) {
      [msg.sender.avatar] = await this._storageService.getDownloadUrls([msg?.sender?.avatar])
    }
